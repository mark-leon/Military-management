...
router
  ...
  // Get Single military
  .get((req, res) => {
    militarySchema.findById(req.params.id, (error, data) => {
      if (error) {
        return next(error);
      } else {
        res.json(data);
...
router.delete("/delete-military/:id", (req, res, next) => {
  militarySchema.findByIdAndRemove(req.params.id, (error, data) => {
    if (error) {
      return next(error);
    } else {
      res.status(200).json({
